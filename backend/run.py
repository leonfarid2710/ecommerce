...
DB_PATH      = '/var/data/ecommerce.db' if os.path.isdir('/var/data') else os.path.join(BASE_DIR, 'ecommerce.db')
...
DOMINIOS_VALIDOS = {
    ...
    'hotmail.es','yahoo.es','gmail.es','uacam.mx','itcampeche.edu.mx',
}
...
def valid_email(email):
    ...
    if dominio not in DOMINIOS_VALIDOS:
        return False, f'Dominio "{dominio}" no valido. Usa Gmail, Hotmail, Outlook, Yahoo, etc.'
    return True, ''
...
def register():
    ...
    return jsonify(message='Registro exitoso.',
                   user={'id': uid, 'nombre': nombre, 'email': email}), 201
...
def checkout():
    ...
    for i in items:
        if i['cantidad'] > i['existencias']:
            db.close()
            return jsonify(error=f'Stock insuficiente para "{i["nombre"]}".'), 409
    total = sum(i['cantidad'] * i['precio'] for i in items)
    cur   = mutate(db, 'INSERT INTO ventas (cliente_id,total,estado,metodo_pago) VALUES (?,?,?,?)',
                   (uid, round(total, 2), 'pagado', mp))
    vid = cur.lastrowid
    for i in items:
        mutate(db, 'INSERT INTO detalle_venta (venta_id,producto_id,cantidad,precio_unitario) VALUES (?,?,?,?)',
    ...
    return jsonify(
        ...
        metodo_pago=mp,
        items=[{'nombre': i['nombre'], 'cantidad': i['cantidad'], 'precio': i['precio']} for i in items]
    ), 201
...
def _page(title, body, active=''):
    ...
    for label, href, key in nav_items:
        ...
        nav_html += (f'<a href="{href}" style="padding:.4rem .9rem;border-radius:6px;'
                     f'font-size:.85rem;text-decoration:none;background:{bg};color:{cl}">{label}</a>')
    css = (
        '<style>'
        ...
        '{padding:.35rem .6rem;background:#0f0e0d;border:1px solid #3a3835;border-radius:7px;color:#faf8f4;font-size:.82rem;font-family:inherit}'
        'input:focus,select:focus,textarea:focus{outline:none;border-color:#d4541a}'
        ...
        '.msg-err{background:#3a1a1a;border:1px solid #7d2e2e;color:#f97676;padding:.65rem 1.1rem;border-radius:9px;margin-bottom:1.25rem;font-size:.88rem}'
        ...
        '.form-field input,.form-field select,.form-field textarea{padding:.55rem .8rem;font-size:.9rem}'
        '</style>')
...
# ─────────────────────────────────────────────────────────────────────────────
#  ADMIN — productos (con formulario para agregar)
# ─────────────────────────────────────────────────────────────────────────────
...
def admin_productos():
    ...
    cats = ['Electronica','Ropa','Accesorios','Artesanias','Calzado','Papeleria','General']
    prov_opts = '<option value="">Sin proveedor</option>' + ''.join(
        '<option value="' + str(p['id']) + '">' + p['nombre'] + '</option>' for p in provs)
    cat_opts = ''.join('<option value="' + c + '">' + c + '</option>' for c in cats)
    add_form = (
        '<div class="add-form"><h3>➕ Agregar nuevo producto</h3>'
        '<form method="POST" action="/admin/productos/agregar">'
        '<div class="form-row">'
        '<div class="form-field"><label>Nombre *</label>'
        '<input type="text" name="nombre" placeholder="Nombre del producto" required style="width:200px"/></div>'
        '<div class="form-field"><label>Precio *</label>'
        '<input type="number" name="precio" placeholder="0.00" step="0.01" min="0.01" required style="width:100px"/></div>'
        '<div class="form-field"><label>Stock inicial</label>'
        '<input type="number" name="existencias" value="0" min="0" style="width:80px"/></div>'
        '<div class="form-field"><label>Categoria</label>'
        '<select name="categoria" style="padding:.55rem .8rem">' + cat_opts + '</select></div>'
        '<div class="form-field"><label>Proveedor</label>'
        '<select name="proveedor_id" style="padding:.55rem .8rem">' + prov_opts + '</select></div>'
        '</div>'
...
def admin_agregar_producto():
    ...
    existencias = int(request.form.get('existencias', 0) or 0)
    categoria   = request.form.get('categoria', 'General').strip()
    imagen_url  = request.form.get('imagen_url', '').strip()
    prov_id     = request.form.get('proveedor_id', '').strip()
    if not nombre or precio <= 0:
        return redirect('/admin/productos?err=Nombre+y+precio+son+obligatorios')
    db = get_db()
    mutate(db,
           'INSERT INTO productos (nombre,descripcion,precio,existencias,proveedor_id,categoria,imagen_url) '
           'VALUES (?,?,?,?,?,?,?)',
           (nombre, descripcion or None, precio, existencias,
            int(prov_id) if prov_id else None, categoria, imagen_url or None))
    db.commit()
    db.close()
    return redirect('/admin/productos?msg=Producto+agregado+correctamente')
...
def admin_proveedores():
    ...
    add_form = (
        ...
        '<div class="form-field"><label>&nbsp;</label>'
        '<button type="submit" class="btn-save" style="padding:.6rem 1.25rem;font-size:.9rem">Agregar</button></div>'
        '</div></form></div>')
    tbody = ''
    for p in rows:
    ...
    body = (f'<h1>Proveedores</h1>'
            ...
            + msg_html + add_form +
            '<table><thead><tr>'
            '<th>ID</th><th>Productos</th><th>Nombre / Editar</th>'
            '<th>Contacto</th><th>Email</th><th>Telefono</th><th>Eliminar</th>'
            f'</tr></thead><tbody>{tbody}</tbody></table>')
...
def admin_ventas():
    ...
    for v in ventas:
        detalles = fetchall(db,
                            ...
                            'JOIN productos p ON dv.producto_id=p.id '
                            'WHERE dv.venta_id=?', (v['id'],))
        items = ', '.join(d['nombre'] + ' x' + str(d['cantidad']) for d in detalles)
        if v['estado'] == 'pagado':
